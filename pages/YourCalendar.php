...
/** Contiene la lingua del calendario per le intestazioni dei giorni e dei mesi
  * @var string */
	...
	function setLanguage($lang)
	{
		...
		die ("Language not supported: only english and italian");
	}	
  ...
  */	
   function setCSSClasses($heading,$simple,$linked,$today,$todayLinked,$weekDays)
   {
	...
	function getDayEvents()
	{
		...
		ORDER BY {$this->tableInfo['dateField']} DESC	  ");
...
/** Ritorna il codice XHTML contenente il calendario intabellato  
  */
	function  getCalendar()
	{
		...
		$result= "<table summary=\"calendario\" border=\"0\" cellspacing=\"3\" cellpadding=\"0\" >";
		...
		$result.= "<td class=\"{$this->CSS['heading']}\" colspan=\"3\" >".($this->lang=='italian'?mese_italiano(date("F",$date_timestamp)):date("F",$date_timestamp))." ".$temp_yr."</td>";
				...
				for ($col=1;$col<=7;$col++)
					{
						...
						if (mysql_num_rows($result_)>0 && ($t == date("j")) && ($this->curDate['month'] == date("n")) && ($this->curDate['year'] == date("Y"))) //alla data di oggi c' un evento
							$result.= "\n<td class=\"{$this->CSS['todayLinked']}\"><a href=\"{$this->linkedPage}?".buildQueryString("cal_{$this->tableInfo['name']}_focus")."&amp;cal_{$this->tableInfo['name']}_focus={$this->curDate['year']}-{$this->curDate['month']}-{$t}\" >".$t."</a></td>";
						
						else if (mysql_num_rows($result_)>0)
							$result.= "\n<td class=\"{$this->CSS['linked']}\"><a href=\"{$this->linkedPage}?".buildQueryString("cal_{$this->tableInfo['name']}_focus")."&amp;cal_{$this->tableInfo['name']}_focus={$this->curDate['year']}-{$this->curDate['month']}-{$t}\" >".$t."</a></td>";
